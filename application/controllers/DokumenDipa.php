...
class DokumenDipa extends CI_Controller {
	var $table_name = 'dokumen_dipa';
		...
		if($method != 'POST'){
			...
			if($check_auth_client == true){
		        	...
		        	if($response['status'] == 200){
						...
						// echo $params['nama'];
						// echo empty('asdf');
						if (empty($params['nama']) || empty($params['url_file_dipa']) || empty($params['url_file_lkk'])) {//ISI NAMA PARAMETER INPUT POST NYA
							$respStatus = 400;
		...
		if($method != 'PUT' || $this->uri->segment(3) == '' || is_numeric($this->uri->segment(3)) == FALSE){
			...
			if($check_auth_client == true){
				...
				if($response['status'] == 200){
					...
					if (empty($params['nama']) || empty($params['url_file_dipa']) || empty($params['url_file_lkk'])) {//CEK PARAMETER INPUT NYA
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Input form masih salah, silahkan coba lagi');
					} else {
