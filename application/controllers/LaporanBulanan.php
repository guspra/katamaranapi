...
class LaporanBulanan extends CI_Controller {
	var $table_name = 'laporan_bulanan';
		...
		if($method != 'POST'){
			...
			if($check_auth_client == true){
		        	...
		        	if($response['status'] == 200){
						...
						if (empty($params['bulan']) || empty($params['tahun']) || empty($params['id_ob']) || empty($params['id_pengawas']) || empty($params['id_pj']) || empty($params['id_kadivmin']) || empty($params['status_ttd'])) {//ISI NAMA PARAMETER INPUT POST NYA
							$respStatus = 400;
		...
		if($method != 'PUT' || $this->uri->segment(3) == '' || is_numeric($this->uri->segment(3)) == FALSE){
			...
			if($check_auth_client == true){
				...
				if($response['status'] == 200){
					...
					if (empty($params['bulan']) || empty($params['tahun']) || empty($params['id_ob']) || empty($params['id_pengawas']) || empty($params['id_pj']) || empty($params['id_kadivmin']) || empty($params['status_ttd'])) { //CEK PARAMETER INPUT NYA
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Input form masih salah, silahkan coba lagi');
					} else {
